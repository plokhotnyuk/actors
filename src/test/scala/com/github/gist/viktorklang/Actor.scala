...
object Actor {
  ...
  def apply(initial: Address => Behavior, batch: Int = 5)(implicit e: Executor): Address = // Seeded by the self-reference that yields the initial behavior
    new AtomicReference[AnyRef]((self: Address) => Become(initial(self))) with Address { // Memory visibility of behavior is guarded by volatile piggybacking & executor
      ...
      private def tryAct(b: Behavior, n: Node, x: Boolean): Unit = if (x) act(b, n, batch) else if ((n ne get) || !compareAndSet(n, b)) actOrSuspend(b, n, 0)
      @tailrec private def actOrSuspend(b: Behavior, n: Node, i: Int): Unit = { val n1 = n.get; if (n1 ne null) act(b, n1, batch) else if (i != 99999) actOrSuspend(b, n, i + 1) else { Thread.`yield`(); async(b, n, false) } }
