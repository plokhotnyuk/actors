...
class FixedThreadPoolExecutorSpec extends Specification {
  ...
  "can execute tasks in parallel threads" in {
    val n = 10 * Runtime.getRuntime.availableProcessors()
    withExecutor(new FixedThreadPoolExecutor(n)) {
      e =>
  ...
  "shutdownNow interrupts threads and return non-completed tasks" in {
    withExecutor(new FixedThreadPoolExecutor(1, onError = { case _: InterruptedException => })) {
      e =>
        ...
        e.execute(task2)
        assertCountDown(latch)
        val al = new java.util.ArrayList[Runnable]()
        al.add(task1)
        al.add(task2)
        e.shutdownNow() must_== al
