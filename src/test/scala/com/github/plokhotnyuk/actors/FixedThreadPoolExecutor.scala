...
private final class Poller(onError: Throwable => Unit,
  ...
  override protected def tryAcquireShared(base: Int): Int = {
    ...
    pollAndRun(workerTail, workerTail, base, 1, batch, size)
  }
  ...
  private def pollAndRun(workerTail: PaddedAtomicReference[TaskNode], tail: PaddedAtomicReference[TaskNode],
    ...
    if (n ne null) {
      if (tail.compareAndSet(tn, n)) {
        ...
        else if (b != 0) pollAndRun(workerTail, workerTail, base, 1, b - 1, size)
        else 0 // slowdown to avoid starvation
      } else pollAndRun(workerTail, workerTail, base, 1, batch, size)
      ...
      if (s == size) emptyCheck(base) = true
      val ns = if (emptyCheck(base)) s - 1 else size
      pollAndRun(workerTail, workerTail, base, 1, batch, ns)
    } else {
      for (i <- 0 until size) emptyCheck(i) = false
      -1
    }
  }
}
