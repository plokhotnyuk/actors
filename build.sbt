...
libraryDependencies ++= Seq(
  "com.typesafe.akka" %% "akka-actor" % "2.3.0-RC3" % "test",
  "net.liftweb" %% "lift-actor" % "2.6-M2" % "test",
  "org.scala-lang" % "scala-actors" % "2.10.4-RC2" % "test",
...
testGrouping <<= definedTests in Test map { tests =>
  tests.map { test =>
    ...
    new Group(
      name = test.name,
      tests = Seq(test),
      runPolicy = SubProcess(javaOptions = Seq("-server", "-Xms6g", "-Xmx6g", "-XX:NewSize=5g", "-Xss228k", "-XX:PermSize=64m", "-XX:MaxPermSize=64m",
