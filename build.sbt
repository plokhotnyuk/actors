...
testGrouping <<= definedTests in Test map { tests =>
  tests.map { test =>
    ...
    new Group(
      name = test.name,
      tests = Seq(test),
      runPolicy = SubProcess(javaOptions = Seq("-server", "-Xms4g", "-Xmx4g", "-XX:NewSize=3g", "-Xss228k", "-XX:PermSize=64m", "-XX:MaxPermSize=64m",
        "-XX:+TieredCompilation", "-XX:+UseParNewGC", "-XX:+UseConcMarkSweepGC", "-XX:+UseNUMA", "-XX:+UseCondCardMark", "-XX:-UseBiasedLocking", "-XX:+AlwaysPreTouch") ++
